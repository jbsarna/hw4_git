...
```{r}
# Created a linear regression for fun to estimate family income with test data set
linear_mod2 <- lm(FamilyIncome ~ Insurance + HouseCosts + ElectricBill + NumWorkers + FamilyType + FoodStamp + OwnRent + NumBedrooms + NumChildren + NumRooms + NumPeople + NumVehicles + Language, data=acs)
summary(linear_mod2)
linear_mod2$fitted.values[1:25]
acs$linest_HighIncome <- ifelse(linear_mod2$fit > 250000,1,0)
