...
# scatter house costs and family income
...
```{r}
# Created a linear regression to estimate family income with test data set
...
```{r}
acs_knn1 <- knn(acsNorm_learn[,2:9], acsNorm_valid[,2:9], acsNorm_learn$HighIncome, k=5, prob = TRUE)
head(acs_knn1)

# Show the classification matrix
table(acsNorm_valid$HighIncome, acs_knn1)
```

```{r}
pcol1 <- as.character(as.numeric(acsNorm_valid$HighIncome1))
pairs(acsNorm_valid[2:9], pch = pcol1, col = c("green3", "red")
  [(acsNorm_valid$HighIncome1 != acs_knn1)+1])
```
```{r}
knn1_pred <- predict(acs_knn1, acsNorm_learn, type="class")
knn1_cm1_pred <- confusionMatrix(knn_pred, acs_test$HighIncome, positive = "1")
```


```{r}
acs_knn2 <- knn(acsNorm_learn[,2:5], acsNorm_valid[,2:5], acsNorm_learn$HighIncome, k=5, prob = TRUE)
head(acs_knn2)
